...
public:
    ...
    std::shared_ptr<ucontext_t> os_context_ptr = std::make_shared<ucontext_t>();
    std::shared_ptr<tcb> current_context_tcb;
    static int cpus;
    int cpu_id;

    static std::deque<cpu*> asleep_cpus;

    static std::deque<std::shared_ptr<tcb>> ready_queue;
    static std::unordered_map<int, std::shared_ptr<tcb>> id_to_tcb;
    static std::unordered_map<int, std::deque<std::shared_ptr<tcb>>> id_to_joined;
    static int next_thread_id;
    static int make_thread(thread_startfunc_t func, uintptr_t arg);

    static void send_ipi();
    static void ipi_handler();
    static void context_switcher(thread_startfunc_t func, uintptr_t arg);
    static bool test_and_set();
